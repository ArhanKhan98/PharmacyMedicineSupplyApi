    ...
    public class SupplyRepository : ISupply
    {
        ...
        public async Task<IEnumerable<PharmacyMedicineSupply>> GetSupplies(string med,int demand)
        {
            ...
            if (inStock > demandStock)
            {
                ...
                for (int i = 0; i < totalPharmacies - 1; i++)
                {
                    medSupply = new PharmacyMedicineSupply() { MedicineName = med, PharmacyName = Pharmacies[i], SupplyCount = medcount };
                    supplies.Add(medSupply);
                    demandStock = demandStock - medcount;
                }
                medSupply = new PharmacyMedicineSupply() { MedicineName = med, PharmacyName = Pharmacies[totalPharmacies - 1], SupplyCount = demandStock };
                supplies.Add(medSupply);
            }
            else
            {
                ...
                medSupply = new PharmacyMedicineSupply() { MedicineName = med, PharmacyName = Pharmacies[totalPharmacies - 1], SupplyCount = inStock };
                supplies.Add(medSupply);
            }
